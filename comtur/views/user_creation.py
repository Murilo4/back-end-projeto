...
def create_user(request):
    ...
    with transaction.atomic():
        if cpf is not None:
            ...
            print(f"Nome recebido: '{userName}'")
            ...
            for nome in name:
                ...
                try:
                    print(f"Tentando buscar: {nome_lower}")
                    ...
                    print(f"Dados para o serializer: {test_data}")
                    if serializer.is_valid():
                        obj = serializer.save()
                        print(f"Nome salvo: {obj.name}")
                        referencias.append(obj.id)
                    else:
...
def cript_password(password):
    ...
    # Gera um salt com o custo padrão
    salt = bcrypt.gensalt(8)
    # Criptografa a senha
    cripted_password = bcrypt.hashpw(encoded_password, salt)
    return cripted_password.decode('utf-8')  # Retorna como string
